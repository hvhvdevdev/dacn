<template>
  <v-row justify="center">
    ...
    <v-col cols="12" sm="10" md="8">
      <v-card>
        <v-card-title>Create `{{ modelName }}`</v-card-title>
        <v-card-text>
          ...
          <v-btn color="primary" :loading="loading" @click="createEntry">
...
export default {
  ...
  methods: {
    async createEntry() {
      this.loading = true
      await this.$store.dispatch('content/createEntry', {
        model: this.modelName,
        data: this.content,
      })

      this.$store.commit('setSnackbarContent', 'Entry created successfully')
